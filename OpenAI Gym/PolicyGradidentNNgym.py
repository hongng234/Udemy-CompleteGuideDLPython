...
for gradient, variable in gradients_and_variables:
    ...
    gradient_placeholder = tf.placeholder(dtype=tf.float32, shape=gradient.get_shape())
    ...
    for iteration in range(num_iterations):
        ...
        for game in range(num_game_rounds):
            ...
            for step in range(max_game_steps):
                ...
                action_val, gradients_val = sess.run([action, gradients], feed_dict={x: observation.reshape(1, num_inputs)})
        ...
        meta_graph_def = tf.train.export_meta_graph(filename='models/my_policy_model.meta')
        saver.save(sess, 'models/my_policy_model')
